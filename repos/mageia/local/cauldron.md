# `mageia:cauldron`

## Docker Metadata

- Image ID: `sha256:b87a6a1118c2bfae4e6cc27e74101537aaad22888734908bab43268554d5b780`
- Created: `2021-04-08T19:23:14.855796341Z`
- Virtual Size: ~ 316.71 Mb  
  (total size of all layers on-disk)
- Arch: `linux`/`amd64`
- Command: `["/bin/bash"]`
- Environment:
  - `PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin`

## `rpm` (`.rpm`-based packages)

### `rpm` package: `basesystem-minimal-8-0.4.mga8.x86_64`

Licenses (from `rpm --query`): GPL

Source:

```console
$ dnf --quiet download --source --url basesystem-minimal-8-0.4.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/basesystem-8-0.4.mga8.src.rpm
```

### `rpm` package: `basesystem-minimal-core-8-0.4.mga8.x86_64`

Licenses (from `rpm --query`): GPL

Source:

```console
$ dnf --quiet download --source --url basesystem-minimal-core-8-0.4.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/basesystem-8-0.4.mga8.src.rpm
```

### `rpm` package: `bash-5.1-4.1.mga8.x86_64`

Licenses (from `rpm --query`): GPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `bzip2-1.0.8-2.mga8.x86_64`

Licenses (from `rpm --query`): BSD

Source:

```console
$ dnf --quiet download --source --url bzip2-1.0.8-2.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/bzip2-1.0.8-2.mga8.src.rpm
```

### `rpm` package: `chkconfig-1.15-1.mga9.x86_64`

Licenses (from `rpm --query`): GPLv2

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `common-licenses-1.1-10.mga8.noarch`

Licenses (from `rpm --query`): GPLv2+

Source:

```console
$ dnf --quiet download --source --url common-licenses-1.1-10.mga8.noarch
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/common-licenses-1.1-10.mga8.src.rpm
```

### `rpm` package: `coreutils-8.32-2.mga8.x86_64`

Licenses (from `rpm --query`): GPLv3+

Source:

```console
$ dnf --quiet download --source --url coreutils-8.32-2.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/coreutils-8.32-2.mga8.src.rpm
```

### `rpm` package: `cpio-2.13-5.mga8.x86_64`

Licenses (from `rpm --query`): GPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `cracklib-dicts-2.9.7-4.mga9.x86_64`

Licenses (from `rpm --query`): LGPLv2

Source:

```console
$ dnf --quiet download --source --url cracklib-dicts-2.9.7-4.mga9
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/cracklib-2.9.7-4.mga9.src.rpm
```

### `rpm` package: `cronie-1.5.5-2.mga8.x86_64`

Licenses (from `rpm --query`): MIT and BSD

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `crontabs-1.10-24.mga8.noarch`

Licenses (from `rpm --query`): GPLv2+

Source:

```console
$ dnf --quiet download --source --url crontabs-1.10-24.mga8.noarch
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/crontabs-1.10-24.mga8.src.rpm
```

### `rpm` package: `crypto-policies-20210118-1.mga8.noarch`

Licenses (from `rpm --query`): LGPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `curl-7.76.0-1.mga9.x86_64`

Licenses (from `rpm --query`): BSD-like

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `dash-static-0.5.11.2-1.mga8.x86_64`

Licenses (from `rpm --query`): BSD

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `dbus-1.13.18-3.mga8.x86_64`

Licenses (from `rpm --query`): GPLv2+ or AFL

Source:

```console
$ dnf --quiet download --source --url dbus-1.13.18-3.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/dbus-1.13.18-3.mga8.src.rpm
```

### `rpm` package: `deltarpm-3.6.2-4.mga9.x86_64`

Licenses (from `rpm --query`): BSD-3-Clause

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `dnf-4.6.0-2.mga9.noarch`

Licenses (from `rpm --query`): GPLv2+ and GPLv2 and GPL

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `dnf-data-4.6.0-2.mga9.noarch`

Licenses (from `rpm --query`): GPLv2+ and GPLv2 and GPL

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `e2fsprogs-1.46.2-2.mga9.x86_64`

Licenses (from `rpm --query`): GPLv2

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `etcskel-1.63-36.mga8.noarch`

Licenses (from `rpm --query`): Public Domain

Source:

```console
$ dnf --quiet download --source --url etcskel-1.63-36.mga8.noarch
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/etcskel-1.63-36.mga8.src.rpm
```

### `rpm` package: `ethtool-5.10-1.mga8.x86_64`

Licenses (from `rpm --query`): GPLv2

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `file-5.40-3.mga9.x86_64`

Licenses (from `rpm --query`): BSD

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `filesystem-2.1.9-34.mga8.x86_64`

Licenses (from `rpm --query`): Public Domain

Source:

```console
$ dnf --quiet download --source --url filesystem-2.1.9-34.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/filesystem-2.1.9-34.mga8.src.rpm
```

### `rpm` package: `findutils-4.8.0-1.mga8.x86_64`

Licenses (from `rpm --query`): GPLv3

Source:

```console
$ dnf --quiet download --source --url findutils-4.8.0-1.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/findutils-4.8.0-1.mga8.src.rpm
```

### `rpm` package: `gawk-5.1.0-1.mga8.x86_64`

Licenses (from `rpm --query`): GPLv3+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `genhdlist2-7.12-2.mga8.noarch`

Licenses (from `rpm --query`): GPLv2+

Source:

```console
$ dnf --quiet download --source --url genhdlist2-7.12-2.mga8.noarch
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/rpmtools-7.12-2.mga8.src.rpm
```

### `rpm` package: `gettext-base-0.21-8.mga8.x86_64`

Licenses (from `rpm --query`): GPLv3+ and LGPLv2+ and GFDL

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `glibc-2.33-7.mga9.x86_64`

Licenses (from `rpm --query`): LGPL

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `gnupg2-2.2.27-3.mga9.x86_64`

Licenses (from `rpm --query`): GPLv3+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `gobject-introspection-1.68.0-1.mga9.x86_64`

Licenses (from `rpm --query`): GPLv2+ and LGPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `gpg-pubkey-80420f66-5d0d4576`

Licenses (from `rpm --query`): pubkey

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `grep-3.6-2.mga8.x86_64`

Licenses (from `rpm --query`): GPLv3

Source:

```console
$ dnf --quiet download --source --url grep-3.6-2.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/grep-3.6-2.mga8.src.rpm
```

### `rpm` package: `gzip-1.10-4.mga8.x86_64`

Licenses (from `rpm --query`): GPLv3+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `hostname-3.23-2.mga8.x86_64`

Licenses (from `rpm --query`): GPLv2+

Source:

```console
$ dnf --quiet download --source --url hostname-3.23-2.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/hostname-3.23-2.mga8.src.rpm
```

### `rpm` package: `icu68-data-68.2-1.mga8.noarch`

Licenses (from `rpm --query`): Unicode

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `ifmetric-0.3-19.mga8.x86_64`

Licenses (from `rpm --query`): GPLv2+

Source:

```console
$ dnf --quiet download --source --url ifmetric-0.3-19.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/ifmetric-0.3-19.mga8.src.rpm
```

### `rpm` package: `ifplugd-0.28-23.mga8.x86_64`

Licenses (from `rpm --query`): GPL

Source:

```console
$ dnf --quiet download --source --url ifplugd-0.28-23.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/ifplugd-0.28-23.mga8.src.rpm
```

### `rpm` package: `initscripts-9.78-27.mga8.x86_64`

Licenses (from `rpm --query`): GPLv2 and GPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `ipcalc-1.0.0-1.mga8.x86_64`

Licenses (from `rpm --query`): GPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `iproute2-5.10.0-1.mga8.x86_64`

Licenses (from `rpm --query`): GPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `iputils-20210202-1.mga9.x86_64`

Licenses (from `rpm --query`): BSD

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `kbd-2.4.0-1.mga8.x86_64`

Licenses (from `rpm --query`): GPLv2+

Source:

```console
$ dnf --quiet download --source --url kbd-2.4.0-1.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/kbd-2.4.0-1.mga8.src.rpm
```

### `rpm` package: `kmod-28-1.mga8.x86_64`

Licenses (from `rpm --query`): GPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `krb5-1.19.1-1.mga9.x86_64`

Licenses (from `rpm --query`): MIT

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `less-575-1.mga9.x86_64`

Licenses (from `rpm --query`): GPLv3+ or BSD-like

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64acl1-2.2.53-2.mga8.x86_64`

Licenses (from `rpm --query`): GPLv2+ and LGPLv2

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64archive13-3.5.1-1.mga8.x86_64`

Licenses (from `rpm --query`): BSD

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64argon2_1-20190702-3.mga8.x86_64`

Licenses (from `rpm --query`): ASL 2.0

Source:

```console
$ dnf --quiet download --source --url lib64argon2_1-20190702-3.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/argon2-20190702-3.mga8.src.rpm
```

### `rpm` package: `lib64assuan0-2.5.4-1.mga8.x86_64`

Licenses (from `rpm --query`): LGPLv3

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64attr1-2.4.48-3.mga8.x86_64`

Licenses (from `rpm --query`): LGPLv2.1

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64audit1-3.0.1-2.mga9.x86_64`

Licenses (from `rpm --query`): LGPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64blkid1-2.36.2-2.mga9.x86_64`

Licenses (from `rpm --query`): LGPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64brotlicommon1-1.0.9-3.mga9.x86_64`

Licenses (from `rpm --query`): MIT

Source:

```console
$ dnf --quiet download --source --url lib64brotlicommon1-1.0.9-3.mga9
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/brotli-1.0.9-3.mga9.src.rpm
```

### `rpm` package: `lib64brotlidec1-1.0.9-3.mga9.x86_64`

Licenses (from `rpm --query`): MIT

Source:

```console
$ dnf --quiet download --source --url lib64brotlidec1-1.0.9-3.mga9
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/brotli-1.0.9-3.mga9.src.rpm
```

### `rpm` package: `lib64bz2_1-1.0.8-2.mga8.x86_64`

Licenses (from `rpm --query`): BSD

Source:

```console
$ dnf --quiet download --source --url lib64bz2_1-1.0.8-2.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/bzip2-1.0.8-2.mga8.src.rpm
```

### `rpm` package: `lib64cap-ng0-0.8.2-2.mga9.x86_64`

Licenses (from `rpm --query`): LGPLv2+

Source:

```console
$ dnf --quiet download --source --url lib64cap-ng0-0.8.2-2.mga9
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/libcap-ng-0.8.2-2.mga9.src.rpm
```

### `rpm` package: `lib64cap2-2.46-1.mga8.x86_64`

Licenses (from `rpm --query`): BSD/GPLv2

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64com_err2-1.46.2-2.mga9.x86_64`

Licenses (from `rpm --query`): MIT

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64comps0-0.1.15-3.mga9.x86_64`

Licenses (from `rpm --query`): GPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64crack2-2.9.7-4.mga9.x86_64`

Licenses (from `rpm --query`): LGPLv2

Source:

```console
$ dnf --quiet download --source --url lib64crack2-2.9.7-4.mga9
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/cracklib-2.9.7-4.mga9.src.rpm
```

### `rpm` package: `lib64cryptsetup12-2.3.5-1.mga9.x86_64`

Licenses (from `rpm --query`): GPLv2+ and LGPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64curl4-7.76.0-1.mga9.x86_64`

Licenses (from `rpm --query`): BSD-like

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64daemon0-0.14-13.mga8.x86_64`

Licenses (from `rpm --query`): LGPLv2+

Source:

```console
$ dnf --quiet download --source --url lib64daemon0-0.14-13.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/libdaemon-0.14-13.mga8.src.rpm
```

### `rpm` package: `lib64db5.3-5.3.28-25.mga8.x86_64`

Licenses (from `rpm --query`): BSD and LGPLv2 and Sleepycat

Source:

```console
$ dnf --quiet download --source --url lib64db5.3-5.3.28-25.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/db53-5.3.28-25.mga8.src.rpm
```

### `rpm` package: `lib64dbus1_3-1.13.18-3.mga8.x86_64`

Licenses (from `rpm --query`): GPLv2+ or AFL

Source:

```console
$ dnf --quiet download --source --url lib64dbus1_3-1.13.18-3.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/dbus-1.13.18-3.mga8.src.rpm
```

### `rpm` package: `lib64devmapper1.02-1.02.175-4.mga9.x86_64`

Licenses (from `rpm --query`): GPLv2 and LGPL2.1

Source:

```console
$ dnf --quiet download --source --url lib64devmapper1.02-1.02.175-4.mga9
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/lvm2-2.03.11-4.mga9.src.rpm
```

### `rpm` package: `lib64dnf2-0.58.0-2.mga9.x86_64`

Licenses (from `rpm --query`): LGPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64elfutils1-0.183-1.mga9.x86_64`

Licenses (from `rpm --query`): GPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64expat1-2.2.10-1.mga8.x86_64`

Licenses (from `rpm --query`): MIT

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64ext2fs2-1.46.2-2.mga9.x86_64`

Licenses (from `rpm --query`): GPLv2

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64fdisk1-2.36.2-2.mga9.x86_64`

Licenses (from `rpm --query`): LGPL2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64ffi7-3.3-2.mga8.x86_64`

Licenses (from `rpm --query`): BSD

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64fuse2-2.9.9-4.mga8.x86_64`

Licenses (from `rpm --query`): GPLv2+

Source:

```console
$ dnf --quiet download --source --url lib64fuse2-2.9.9-4.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/fuse-2.9.9-4.mga8.src.rpm
```

### `rpm` package: `lib64gcrypt20-1.8.7-1.mga8.x86_64`

Licenses (from `rpm --query`): LGPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64gdbm6-1.19-1.mga8.x86_64`

Licenses (from `rpm --query`): GPLv3

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64gdbm_compat4-1.19-1.mga8.x86_64`

Licenses (from `rpm --query`): GPLv3

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64gio2.0_0-2.68.0-1.mga9.x86_64`

Licenses (from `rpm --query`): LGPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64girepository1.0_1-1.68.0-1.mga9.x86_64`

Licenses (from `rpm --query`): GPLv2+ and LGPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64glib-gir2.0-1.68.0-1.mga9.x86_64`

Licenses (from `rpm --query`): GPLv2+ and LGPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64glib2.0_0-2.68.0-1.mga9.x86_64`

Licenses (from `rpm --query`): LGPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64gmp10-6.2.1-1.mga8.x86_64`

Licenses (from `rpm --query`): GPLv3

Source:

```console
$ dnf --quiet download --source --url lib64gmp10-6.2.1-1.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/gmp-6.2.1-1.mga8.src.rpm
```

### `rpm` package: `lib64gnutls30-3.6.15-3.mga8.x86_64`

Licenses (from `rpm --query`): LGPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64gpg-error0-1.41-1.mga8.x86_64`

Licenses (from `rpm --query`): LGPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64gpgme11-1.15.1-2.mga9.x86_64`

Licenses (from `rpm --query`): GPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64hogweed6-3.6-1.mga8.x86_64`

Licenses (from `rpm --query`): LGPLv3+ or GPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64icu68-68.2-1.mga8.x86_64`

Licenses (from `rpm --query`): Unicode

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64idn2_0-2.3.0-4.mga8.x86_64`

Licenses (from `rpm --query`): LGPLv3+ or GPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64intl8-0.21-8.mga8.x86_64`

Licenses (from `rpm --query`): LGPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64ip4tc2-1.8.7-1.mga8.x86_64`

Licenses (from `rpm --query`): GPLv2+

Source:

```console
$ dnf --quiet download --source --url lib64ip4tc2-1.8.7-1.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/iptables-1.8.7-1.mga8.src.rpm
```

### `rpm` package: `lib64iptables12-1.8.7-1.mga8.x86_64`

Licenses (from `rpm --query`): GPLv2+

Source:

```console
$ dnf --quiet download --source --url lib64iptables12-1.8.7-1.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/iptables-1.8.7-1.mga8.src.rpm
```

### `rpm` package: `lib64json-c5-0.15-1.mga8.x86_64`

Licenses (from `rpm --query`): MIT

Source:

```console
$ dnf --quiet download --source --url lib64json-c5-0.15-1.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/json-c-0.15-1.mga8.src.rpm
```

### `rpm` package: `lib64keyutils1-1.6.3-1.mga8.x86_64`

Licenses (from `rpm --query`): LGPLv2+

Source:

```console
$ dnf --quiet download --source --url lib64keyutils1-1.6.3-1.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/keyutils-1.6.3-1.mga8.src.rpm
```

### `rpm` package: `lib64kmod2-28-1.mga8.x86_64`

Licenses (from `rpm --query`): LGPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64krb53-1.19.1-1.mga9.x86_64`

Licenses (from `rpm --query`): MIT

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64ksba8-1.5.0-1.mga8.x86_64`

Licenses (from `rpm --query`): GPLv3

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64ldap2.4_2-2.4.58-1.mga9.x86_64`

Licenses (from `rpm --query`): OpenLDAP

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64lua5.4-5.4.3-4.mga9.x86_64`

Licenses (from `rpm --query`): MIT

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64lz4_1-1.9.3-1.mga8.x86_64`

Licenses (from `rpm --query`): BSD

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64lzma5-5.2.5-2.mga8.x86_64`

Licenses (from `rpm --query`): GPLv2+ and Public Domain

Source:

```console
$ dnf --quiet download --source --url lib64lzma5-5.2.5-2.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/xz-5.2.5-2.mga8.src.rpm
```

### `rpm` package: `lib64lzo2_2-2.10-4.mga8.x86_64`

Licenses (from `rpm --query`): GPLv2

Source:

```console
$ dnf --quiet download --source --url lib64lzo2_2-2.10-4.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/liblzo-2.10-4.mga8.src.rpm
```

### `rpm` package: `lib64magic1-5.40-3.mga9.x86_64`

Licenses (from `rpm --query`): BSD

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64microhttpd12-0.9.72-1.mga8.x86_64`

Licenses (from `rpm --query`): LGPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64mnl0-1.0.4-3.mga8.x86_64`

Licenses (from `rpm --query`): LGPLv2+

Source:

```console
$ dnf --quiet download --source --url lib64mnl0-1.0.4-3.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/libmnl-1.0.4-3.mga8.src.rpm
```

### `rpm` package: `lib64modulemd-gir2.0-2.12.0-2.mga9.x86_64`

Licenses (from `rpm --query`): MIT

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64modulemd2-2.12.0-2.mga9.x86_64`

Licenses (from `rpm --query`): MIT

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64mount1-2.36.2-2.mga9.x86_64`

Licenses (from `rpm --query`): LGPL2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64mpfr6-4.1.0-1.mga8.x86_64`

Licenses (from `rpm --query`): LGPLv3+

Source:

```console
$ dnf --quiet download --source --url lib64mpfr6-4.1.0-1.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/mpfr-4.1.0-1.mga8.src.rpm
```

### `rpm` package: `lib64ncurses6-6.2-20201205.1.mga8.x86_64`

Licenses (from `rpm --query`): MIT

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64ncursesw6-6.2-20201205.1.mga8.x86_64`

Licenses (from `rpm --query`): MIT

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64nettle8-3.6-1.mga8.x86_64`

Licenses (from `rpm --query`): LGPLv3+ or GPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64nghttp2_14-1.43.0-1.mga9.x86_64`

Licenses (from `rpm --query`): MIT

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64npth0-1.6-2.mga8.x86_64`

Licenses (from `rpm --query`): LGPLv2+

Source:

```console
$ dnf --quiet download --source --url lib64npth0-1.6-2.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/npth-1.6-2.mga8.src.rpm
```

### `rpm` package: `lib64nsl2-1.3.0-2.mga8.x86_64`

Licenses (from `rpm --query`): BSD and LGPLv2+

Source:

```console
$ dnf --quiet download --source --url lib64nsl2-1.3.0-2.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/libnsl-1.3.0-2.mga8.src.rpm
```

### `rpm` package: `lib64openssl1.1-1.1.1k-1.mga9.x86_64`

Licenses (from `rpm --query`): BSD-like

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64p11-kit0-0.23.22-2.mga8.x86_64`

Licenses (from `rpm --query`): Apache License

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64pam0-1.3.1-7.mga8.x86_64`

Licenses (from `rpm --query`): BSD and GPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64pam_userpass1-1.0.2-15.mga8.x86_64`

Licenses (from `rpm --query`): relaxed BSD and (L)GPL-compatible

Source:

```console
$ dnf --quiet download --source --url lib64pam_userpass1-1.0.2-15.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/pam_userpass-1.0.2-15.mga8.src.rpm
```

### `rpm` package: `lib64pcre1-8.44-1.mga8.x86_64`

Licenses (from `rpm --query`): BSD-Style

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64pcre2_0-10.36-1.mga8.x86_64`

Licenses (from `rpm --query`): BSD

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64pkgconf3-1.7.3-2.mga8.x86_64`

Licenses (from `rpm --query`): ISC

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64popt0-1.18-1.mga8.x86_64`

Licenses (from `rpm --query`): MIT

Source:

```console
$ dnf --quiet download --source --url lib64popt0-1.18-1.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/popt-1.18-1.mga8.src.rpm
```

### `rpm` package: `lib64procps8-3.3.17-1.mga9.x86_64`

Licenses (from `rpm --query`): LGPLv2+

Source:

```console
$ dnf --quiet download --source --url lib64procps8-3.3.17-1.mga9
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/procps-ng-3.3.17-1.mga9.src.rpm
```

### `rpm` package: `lib64psl5-0.21.1-1.mga8.x86_64`

Licenses (from `rpm --query`): MIT

Source:

```console
$ dnf --quiet download --source --url lib64psl5-0.21.1-1.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/libpsl-0.21.1-1.mga8.src.rpm
```

### `rpm` package: `lib64python3.9-3.9.4-1.mga9.x86_64`

Licenses (from `rpm --query`): Modified CNRI Open Source License

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64python3.9-stdlib-3.9.4-1.mga9.x86_64`

Licenses (from `rpm --query`): Modified CNRI Open Source License

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64readline8-8.1-1.mga8.x86_64`

Licenses (from `rpm --query`): GPLv3

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64repo0-1.13.0-1.mga8.x86_64`

Licenses (from `rpm --query`): LGPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64rpm9-4.16.1.3-2.mga9.x86_64`

Licenses (from `rpm --query`): GPLv2+ and LGPLv2+ with exceptions

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64rpmbuild9-4.16.1.3-2.mga9.x86_64`

Licenses (from `rpm --query`): GPLv2+ and LGPLv2+ with exceptions

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64rpmsign9-4.16.1.3-2.mga9.x86_64`

Licenses (from `rpm --query`): GPLv2+ and LGPLv2+ with exceptions

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64sasl2_3-2.1.27-3.mga8.x86_64`

Licenses (from `rpm --query`): BSD-style

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64seccomp2-2.5.1-1.mga8.x86_64`

Licenses (from `rpm --query`): LGPLv2

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64selinux1-3.2-0.rc1.5.mga9.x86_64`

Licenses (from `rpm --query`): Public Domain

Source:

```console
$ dnf --quiet download --source --url lib64selinux1-3.2-0.rc1.5.mga9
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/libselinux-3.2-0.rc1.5.mga9.src.rpm
```

### `rpm` package: `lib64sepol2-3.2-0.rc1.4.mga8.x86_64`

Licenses (from `rpm --query`): LGPLv2+

Source:

```console
$ dnf --quiet download --source --url lib64sepol2-3.2-0.rc1.4.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/libsepol-3.2-0.rc1.4.mga8.src.rpm
```

### `rpm` package: `lib64sigsegv2-2.13-1.mga9.x86_64`

Licenses (from `rpm --query`): GPLv2+

Source:

```console
$ dnf --quiet download --source --url lib64sigsegv2-2.13-1.mga9
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/libsigsegv-2.13-1.mga9.src.rpm
```

### `rpm` package: `lib64smartcols1-2.36.2-2.mga9.x86_64`

Licenses (from `rpm --query`): LGPL2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64solv1-0.7.16-1.mga8.x86_64`

Licenses (from `rpm --query`): BSD

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64sqlite3_0-3.35.4-1.mga9.x86_64`

Licenses (from `rpm --query`): Public Domain

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64ss2-1.46.2-2.mga9.x86_64`

Licenses (from `rpm --query`): MIT

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64ssh4-0.9.5-1.mga8.x86_64`

Licenses (from `rpm --query`): LGPLv2.1+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64systemd0-248-2.mga9.x86_64`

Licenses (from `rpm --query`): GPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64tasn1_6-4.16.0-4.mga8.x86_64`

Licenses (from `rpm --query`): LGPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64tcb0-1.1-12.mga8.x86_64`

Licenses (from `rpm --query`): BSD or GPL+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64tirpc3-1.3.1-1.mga8.x86_64`

Licenses (from `rpm --query`): GPLv2

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64udev1-248-2.mga9.x86_64`

Licenses (from `rpm --query`): GPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64unistring2-0.9.10-4.mga8.x86_64`

Licenses (from `rpm --query`): LGPLv3+ or GPLv2

Source:

```console
$ dnf --quiet download --source --url lib64unistring2-0.9.10-4.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/libunistring-0.9.10-4.mga8.src.rpm
```

### `rpm` package: `lib64usb1.0_0-1.0.24-3.mga9.x86_64`

Licenses (from `rpm --query`): LGPLv2+

Source:

```console
$ dnf --quiet download --source --url lib64usb1.0_0-1.0.24-3.mga9
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/libusb-1.0.24-3.mga9.src.rpm
```

### `rpm` package: `lib64user1-0.63-1.mga9.x86_64`

Licenses (from `rpm --query`): LGPLv2+

Source:

```console
$ dnf --quiet download --source --url lib64user1-0.63-1.mga9
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/libuser-0.63-1.mga9.src.rpm
```

### `rpm` package: `lib64uuid1-2.36.2-2.mga9.x86_64`

Licenses (from `rpm --query`): BSD

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64verto1-0.3.2-1.mga9.x86_64`

Licenses (from `rpm --query`): MIT

Source:

```console
$ dnf --quiet download --source --url lib64verto1-0.3.2-1.mga9
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/libverto-0.3.2-1.mga9.src.rpm
```

### `rpm` package: `lib64xcrypt1-4.4.18-1.mga9.x86_64`

Licenses (from `rpm --query`): LGPL-2.1-or-later AND BSD-2-Clause AND BSD-3-Clause AND SUSE-Public-Domain

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64xml2_2-2.9.10-8.mga9.x86_64`

Licenses (from `rpm --query`): MIT

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64yaml0_2-0.2.5-1.mga8.x86_64`

Licenses (from `rpm --query`): MIT

Source:

```console
$ dnf --quiet download --source --url lib64yaml0_2-0.2.5-1.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/yaml-0.2.5-1.mga8.src.rpm
```

### `rpm` package: `lib64zck1-1.1.9-1.mga8.x86_64`

Licenses (from `rpm --query`): BSD and MIT

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64zlib1-1.2.11-9.mga8.x86_64`

Licenses (from `rpm --query`): BSD

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lib64zstd1-1.4.9-1.mga9.x86_64`

Licenses (from `rpm --query`): BSD and GPLv2

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `libgcc1-10.3.0-0.RC.2.mga9.x86_64`

Licenses (from `rpm --query`): GPLv3+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `libgomp1-10.3.0-0.RC.2.mga9.x86_64`

Licenses (from `rpm --query`): GPLv3+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `libgpg-error-common-1.41-1.mga8.noarch`

Licenses (from `rpm --query`): LGPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `libidn2-i18n-2.3.0-4.mga8.noarch`

Licenses (from `rpm --query`): (GPLv2+ or LGPLv3+) and GPLv3+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `libselinux-3.2-0.rc1.5.mga9.x86_64`

Licenses (from `rpm --query`): Public Domain

Source:

```console
$ dnf --quiet download --source --url libselinux-3.2-0.rc1.5.mga9
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/libselinux-3.2-0.rc1.5.mga9.src.rpm
```

### `rpm` package: `libstdc++6-10.3.0-0.RC.2.mga9.x86_64`

Licenses (from `rpm --query`): GPLv3+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `libtirpc-1.3.1-1.mga8.x86_64`

Licenses (from `rpm --query`): GPLv2

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `libuser-0.63-1.mga9.x86_64`

Licenses (from `rpm --query`): LGPLv2+

Source:

```console
$ dnf --quiet download --source --url libuser-0.63-1.mga9
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/libuser-0.63-1.mga9.src.rpm
```

### `rpm` package: `libutempter-1.2.1-1.mga8.x86_64`

Licenses (from `rpm --query`): LGPLv2+

Source:

```console
$ dnf --quiet download --source --url libutempter-1.2.1-1.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/libutempter-1.2.1-1.mga8.src.rpm
```

### `rpm` package: `locales-2.33-1.mga9.x86_64`

Licenses (from `rpm --query`): GPL

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `locales-en-2.33-1.mga9.x86_64`

Licenses (from `rpm --query`): GPL

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `logrotate-3.17.0-3.mga8.x86_64`

Licenses (from `rpm --query`): GPLv2

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `lsb-release-3.1-3.mga9.noarch`

Licenses (from `rpm --query`): GPL

Source:

```console
$ dnf --quiet download --source --url lsb-release-3.1-3.mga9.noarch
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/lsb-release-3.1-3.mga9.src.rpm
```

### `rpm` package: `mageia-dnf-conf-1.0-3.mga8.noarch`

Licenses (from `rpm --query`): CC0

Source:

```console
$ dnf --quiet download --source --url mageia-dnf-conf-1.0-3.mga8.noarch
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/mageia-dnf-conf-1.0-3.mga8.src.rpm
```

### `rpm` package: `mageia-release-Default-9-0.4.mga9.x86_64`

Licenses (from `rpm --query`): GPL

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `mageia-release-common-9-0.4.mga9.x86_64`

Licenses (from `rpm --query`): GPL

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `mageia-repos-9-0.0.1.mga8.x86_64`

Licenses (from `rpm --query`): MIT

Source:

```console
$ dnf --quiet download --source --url mageia-repos-9-0.0.1.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/mageia-repos-9-0.0.1.mga8.src.rpm
```

### `rpm` package: `mageia-repos-cauldron-9-0.0.1.mga8.x86_64`

Licenses (from `rpm --query`): MIT

Source:

```console
$ dnf --quiet download --source --url mageia-repos-cauldron-9-0.0.1.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/mageia-repos-9-0.0.1.mga8.src.rpm
```

### `rpm` package: `mageia-repos-keys-9-0.0.1.mga8.noarch`

Licenses (from `rpm --query`): MIT

Source:

```console
$ dnf --quiet download --source --url mageia-repos-keys-9-0.0.1.mga8.noarch
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/mageia-repos-9-0.0.1.mga8.src.rpm
```

### `rpm` package: `mageia-repos-pkgprefs-9-0.0.1.mga8.noarch`

Licenses (from `rpm --query`): MIT

Source:

```console
$ dnf --quiet download --source --url mageia-repos-pkgprefs-9-0.0.1.mga8.noarch
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/mageia-repos-9-0.0.1.mga8.src.rpm
```

### `rpm` package: `makedev-4.4-26.mga8.noarch`

Licenses (from `rpm --query`): GPLv2+

Source:

```console
$ dnf --quiet download --source --url makedev-4.4-26.mga8.noarch
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/makedev-4.4-26.mga8.src.rpm
```

### `rpm` package: `meta-task-8-1.mga8.noarch`

Licenses (from `rpm --query`): GPL

Source:

```console
$ dnf --quiet download --source --url meta-task-8-1.mga8.noarch
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/meta-task-8-1.mga8.src.rpm
```

### `rpm` package: `ncurses-6.2-20201205.1.mga8.x86_64`

Licenses (from `rpm --query`): MIT

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `net-tools-2.0-0.20150915git.6.mga8.x86_64`

Licenses (from `rpm --query`): GPLv2+

Source:

```console
$ dnf --quiet download --source --url net-tools-2.0-0.20150915git.6.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/net-tools-2.0-0.20150915git.6.mga8.src.rpm
```

### `rpm` package: `nss-myhostname-248-2.mga9.x86_64`

Licenses (from `rpm --query`): GPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `nss_tcb-1.1-12.mga8.x86_64`

Licenses (from `rpm --query`): BSD or GPL+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `openldap-2.4.58-1.mga9.x86_64`

Licenses (from `rpm --query`): OpenLDAP

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `p11-kit-0.23.22-2.mga8.x86_64`

Licenses (from `rpm --query`): Apache License

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `p11-kit-trust-0.23.22-2.mga8.x86_64`

Licenses (from `rpm --query`): Apache License

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `pam-1.3.1-7.mga8.x86_64`

Licenses (from `rpm --query`): BSD and GPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `pam_tcb-1.1-12.mga8.x86_64`

Licenses (from `rpm --query`): BSD or GPL+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `passwd-0.80-2.mga8.x86_64`

Licenses (from `rpm --query`): BSD

Source:

```console
$ dnf --quiet download --source --url passwd-0.80-2.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/passwd-0.80-2.mga8.src.rpm
```

### `rpm` package: `perl-5.32.1-4.mga9.x86_64`

Licenses (from `rpm --query`): GPL+ or Artistic

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `perl-Config-IniFiles-3.0.3-1.mga8.noarch`

Licenses (from `rpm --query`): GPL

Source:

```console
$ dnf --quiet download --source --url perl-Config-IniFiles-3.0.3-1.mga8.noarch
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/perl-Config-IniFiles-3.0.3-1.mga8.src.rpm
```

### `rpm` package: `perl-File-Sync-0.110.0-31.mga8.x86_64`

Licenses (from `rpm --query`): Artistic

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `perl-Filesys-Df-0.920.0-31.mga8.x86_64`

Licenses (from `rpm --query`): GPL+ or Artistic

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `perl-IO-stringy-2.113.0-1.mga8.noarch`

Licenses (from `rpm --query`): Artistic/GPL

Source:

```console
$ dnf --quiet download --source --url perl-IO-stringy-2.113.0-1.mga8.noarch
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/perl-IO-stringy-2.113.0-1.mga8.src.rpm
```

### `rpm` package: `perl-Locale-gettext-1.70.0-13.mga8.x86_64`

Licenses (from `rpm --query`): GPL+ or Artistic

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `perl-MDK-Common-1.2.34.2-4.mga8.noarch`

Licenses (from `rpm --query`): GPLv2+

Source:

```console
$ dnf --quiet download --source --url perl-MDK-Common-1.2.34.2-4.mga8.noarch
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/perl-MDK-Common-1.2.34.2-4.mga8.src.rpm
```

### `rpm` package: `perl-MDV-Distribconf-4.101.0-3.mga8.noarch`

Licenses (from `rpm --query`): GPL

Source:

```console
$ dnf --quiet download --source --url perl-MDV-Distribconf-4.101.0-3.mga8.noarch
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/perl-MDV-Distribconf-4.101.0-3.mga8.src.rpm
```

### `rpm` package: `perl-MDV-Packdrakeng-1.130.0-6.mga8.noarch`

Licenses (from `rpm --query`): GPL

Source:

```console
$ dnf --quiet download --source --url perl-MDV-Packdrakeng-1.130.0-6.mga8.noarch
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/perl-MDV-Packdrakeng-1.130.0-6.mga8.src.rpm
```

### `rpm` package: `perl-Time-ZoneInfo-0.300.0-10.mga8.noarch`

Licenses (from `rpm --query`): GPL+ or Artistic

Source:

```console
$ dnf --quiet download --source --url perl-Time-ZoneInfo-0.300.0-10.mga8.noarch
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/perl-Time-ZoneInfo-0.300.0-10.mga8.src.rpm
```

### `rpm` package: `perl-URPM-5.123-1.mga8.x86_64`

Licenses (from `rpm --query`): GPL or Artistic

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `perl-XML-LibXML-2.20.600-1.mga8.x86_64`

Licenses (from `rpm --query`): GPL+ or Artistic

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `perl-XML-NamespaceSupport-1.120.0-3.mga8.noarch`

Licenses (from `rpm --query`): MPL

Source:

```console
$ dnf --quiet download --source --url perl-XML-NamespaceSupport-1.120.0-3.mga8.noarch
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/perl-XML-NamespaceSupport-1.120.0-3.mga8.src.rpm
```

### `rpm` package: `perl-XML-SAX-1.20.0-2.mga8.noarch`

Licenses (from `rpm --query`): GPL+ or Artistic

Source:

```console
$ dnf --quiet download --source --url perl-XML-SAX-1.20.0-2.mga8.noarch
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/perl-XML-SAX-1.20.0-2.mga8.src.rpm
```

### `rpm` package: `perl-XML-SAX-Base-1.90.0-4.mga8.noarch`

Licenses (from `rpm --query`): GPL+ or Artistic

Source:

```console
$ dnf --quiet download --source --url perl-XML-SAX-Base-1.90.0-4.mga8.noarch
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/perl-XML-SAX-Base-1.90.0-4.mga8.src.rpm
```

### `rpm` package: `perl-base-5.32.1-4.mga9.x86_64`

Licenses (from `rpm --query`): GPL+ or Artistic

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `pkgconf-1.7.3-2.mga8.x86_64`

Licenses (from `rpm --query`): ISC

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `pkgconf-m4-1.7.3-2.mga8.noarch`

Licenses (from `rpm --query`): GPLv2+ with exceptions

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `pkgconf-pkg-config-1.7.3-2.mga8.x86_64`

Licenses (from `rpm --query`): ISC

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `popt-data-1.18-1.mga8.noarch`

Licenses (from `rpm --query`): MIT

Source:

```console
$ dnf --quiet download --source --url popt-data-1.18-1.mga8.noarch
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/popt-1.18-1.mga8.src.rpm
```

### `rpm` package: `procps-ng-3.3.17-1.mga9.x86_64`

Licenses (from `rpm --query`): GPLv2+ and LGPLv2+

Source:

```console
$ dnf --quiet download --source --url procps-ng-3.3.17-1.mga9
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/procps-ng-3.3.17-1.mga9.src.rpm
```

### `rpm` package: `psmisc-23.4-1.mga9.x86_64`

Licenses (from `rpm --query`): GPLv2+

Source:

```console
$ dnf --quiet download --source --url psmisc-23.4-1.mga9
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/psmisc-23.4-1.mga9.src.rpm
```

### `rpm` package: `publicsuffix-list-dafsa-20201130-1.mga8.noarch`

Licenses (from `rpm --query`): MPLv2.0

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `python-pip-wheel-20.3.3-5.mga9.noarch`

Licenses (from `rpm --query`): MIT

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `python-rpm-macros-3.9-7.mga9.noarch`

Licenses (from `rpm --query`): MIT and Python

Source:

```console
$ dnf --quiet download --source --url python-rpm-macros-3.9-7.mga9.noarch
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/python-rpm-macros-3.9-7.mga9.src.rpm
```

### `rpm` package: `python-setuptools-wheel-50.3.2-5.mga9.noarch`

Licenses (from `rpm --query`): MIT

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `python-srpm-macros-3.9-7.mga9.noarch`

Licenses (from `rpm --query`): MIT and Python

Source:

```console
$ dnf --quiet download --source --url python-srpm-macros-3.9-7.mga9.noarch
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/python-rpm-macros-3.9-7.mga9.src.rpm
```

### `rpm` package: `python3-3.9.4-1.mga9.x86_64`

Licenses (from `rpm --query`): Modified CNRI Open Source License

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `python3-dnf-4.6.0-2.mga9.noarch`

Licenses (from `rpm --query`): GPLv2+ and GPLv2 and GPL

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `python3-gobject-base-3.40.1-1.mga9.x86_64`

Licenses (from `rpm --query`): LGPLv2+ and MIT

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `python3-gpg-1.15.1-2.mga9.x86_64`

Licenses (from `rpm --query`): GPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `python3-hawkey-0.58.0-2.mga9.x86_64`

Licenses (from `rpm --query`): LGPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `python3-libcomps-0.1.15-3.mga9.x86_64`

Licenses (from `rpm --query`): GPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `python3-libdnf-0.58.0-2.mga9.x86_64`

Licenses (from `rpm --query`): LGPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `python3-libmodulemd-2.12.0-2.mga9.x86_64`

Licenses (from `rpm --query`): MIT

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `python3-rpm-4.16.1.3-2.mga9.x86_64`

Licenses (from `rpm --query`): GPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `python3-rpm-macros-3.9-7.mga9.noarch`

Licenses (from `rpm --query`): MIT and Python

Source:

```console
$ dnf --quiet download --source --url python3-rpm-macros-3.9-7.mga9.noarch
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/python-rpm-macros-3.9-7.mga9.src.rpm
```

### `rpm` package: `python3-six-1.15.0-4.mga9.noarch`

Licenses (from `rpm --query`): MIT

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `resolvconf-1.79-5.mga8.noarch`

Licenses (from `rpm --query`): GPLv2+

Source:

```console
$ dnf --quiet download --source --url resolvconf-1.79-5.mga8.noarch
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/resolvconf-1.79-5.mga8.src.rpm
```

### `rpm` package: `rmt-0.4b47-1.mga9.x86_64`

Licenses (from `rpm --query`): BSD

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `rootcerts-20210223.00-1.mga9.noarch`

Licenses (from `rpm --query`): GPL

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `rootfiles-11.0-16.mga8.noarch`

Licenses (from `rpm --query`): Public Domain

Source:

```console
$ dnf --quiet download --source --url rootfiles-11.0-16.mga8.noarch
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/rootfiles-11.0-16.mga8.src.rpm
```

### `rpm` package: `rpm-4.16.1.3-2.mga9.x86_64`

Licenses (from `rpm --query`): GPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `rpm-helper-0.24.21-1.mga8.noarch`

Licenses (from `rpm --query`): GPL

Source:

```console
$ dnf --quiet download --source --url rpm-helper-0.24.21-1.mga8.noarch
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/rpm-helper-0.24.21-1.mga8.src.rpm
```

### `rpm` package: `rpm-mageia-setup-2.58-4.mga9.x86_64`

Licenses (from `rpm --query`): GPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `run-parts-4.8.4-3.mga8.x86_64`

Licenses (from `rpm --query`): GPLv2+

Source:

```console
$ dnf --quiet download --source --url run-parts-4.8.4-3.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/run-parts-4.8.4-3.mga8.src.rpm
```

### `rpm` package: `sash-3.8-9.mga8.x86_64`

Licenses (from `rpm --query`): LGPLv2+

Source:

```console
$ dnf --quiet download --source --url sash-3.8-9.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/sash-3.8-9.mga8.src.rpm
```

### `rpm` package: `sed-4.8-3.mga8.x86_64`

Licenses (from `rpm --query`): GPL

Source:

```console
$ dnf --quiet download --source --url sed-4.8-3.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/sed-4.8-3.mga8.src.rpm
```

### `rpm` package: `setup-2.7.25-1.mga8.noarch`

Licenses (from `rpm --query`): Public Domain

Source:

```console
$ dnf --quiet download --source --url setup-2.7.25-1.mga8.noarch
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/setup-2.7.25-1.mga8.src.rpm
```

### `rpm` package: `shadow-utils-4.6-4.mga8.x86_64`

Licenses (from `rpm --query`): BSD

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `shared-mime-info-2.1-2.mga8.x86_64`

Licenses (from `rpm --query`): GPL+ and LGPLv2+

Source:

```console
$ dnf --quiet download --source --url shared-mime-info-2.1-2.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/shared-mime-info-2.1-2.mga8.src.rpm
```

### `rpm` package: `systemd-248-2.mga9.x86_64`

Licenses (from `rpm --query`): GPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `tar-1.34-1.mga9.x86_64`

Licenses (from `rpm --query`): GPLv3

Source:

```console
$ dnf --quiet download --source --url tar-1.34-1.mga9
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/tar-1.34-1.mga9.src.rpm
```

### `rpm` package: `tcb-1.1-12.mga8.x86_64`

Licenses (from `rpm --query`): BSD or GPL+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `time-1.9-4.mga8.x86_64`

Licenses (from `rpm --query`): GPLv2+

Source:

```console
$ dnf --quiet download --source --url time-1.9-4.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/time-1.9-4.mga8.src.rpm
```

### `rpm` package: `timezone-2021a-1.mga8.x86_64`

Licenses (from `rpm --query`): GPL

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `traceroute-2.1.0-3.mga8.x86_64`

Licenses (from `rpm --query`): GPLv2+

Source:

```console
$ dnf --quiet download --source --url traceroute-2.1.0-3.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/traceroute-2.1.0-3.mga8.src.rpm
```

### `rpm` package: `urpmi-8.125-1.mga8.noarch`

Licenses (from `rpm --query`): GPLv2+

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `util-linux-2.36.2-2.mga9.x86_64`

Licenses (from `rpm --query`): GPLv2 and GPLv2+ and LGPLv2+ and BSD with advertising and Public Domain

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `vim-minimal-8.2.2637-2.mga9.x86_64`

Licenses (from `rpm --query`): Charityware

**WARNING:** unable to find source (`dnf download` failed or returned no results)!

### `rpm` package: `which-2.21-6.mga8.x86_64`

Licenses (from `rpm --query`): GPLv3

Source:

```console
$ dnf --quiet download --source --url which-2.21-6.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/which-2.21-6.mga8.src.rpm
```

### `rpm` package: `xz-5.2.5-2.mga8.x86_64`

Licenses (from `rpm --query`): GPLv2+ and Public Domain

Source:

```console
$ dnf --quiet download --source --url xz-5.2.5-2.mga8
ftp://fr2.rpmfind.net/linux/mageia/distrib/cauldron/SRPMS/core/release/xz-5.2.5-2.mga8.src.rpm
```

### `rpm` package: `zstd-1.4.9-1.mga9.x86_64`

Licenses (from `rpm --query`): BSD and GPLv2

**WARNING:** unable to find source (`dnf download` failed or returned no results)!
