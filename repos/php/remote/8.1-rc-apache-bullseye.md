...
```console
$ docker pull php@sha256:f887775aaed3074ae0d3db3d712458cc2cc5ef88ecf4ae294bb72603ff87160a
```
...
```console
$ docker pull php@sha256:3d21af97d02bf1568b2c456bbe465704949f521008b835dac547a6bc5d1b930e
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **142.7 MB (142725347 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fcc98207fabfffdfdda8f294bc9786afc535231b2a6edee59fb4251b4df99de1`
-	Entrypoint: `["docker-php-entrypoint"]`
-	Default Command: `["apache2-foreground"]`
...
ENV PHP_VERSION=8.1.18RC1
# Wed, 12 Apr 2023 19:54:13 GMT
ENV PHP_URL=https://downloads.php.net/~ramsey/php-8.1.18RC1.tar.xz PHP_ASC_URL=https://downloads.php.net/~ramsey/php-8.1.18RC1.tar.xz.asc
# Wed, 12 Apr 2023 19:54:17 GMT
ENV PHP_SHA256=b1fe3a2be6085122d976f93513b397108767b552e0011813a5314b52e108c3fd
# Wed, 12 Apr 2023 19:55:04 GMT
RUN set -eux; 		savedAptMark="$(apt-mark showmanual)"; 	apt-get update; 	apt-get install -y --no-install-recommends gnupg dirmngr; 	rm -rf /var/lib/apt/lists/*; 		mkdir -p /usr/src; 	cd /usr/src; 		curl -fsSL -o php.tar.xz "$PHP_URL"; 		if [ -n "$PHP_SHA256" ]; then 		echo "$PHP_SHA256 *php.tar.xz" | sha256sum -c -; 	fi; 		if [ -n "$PHP_ASC_URL" ]; then 		curl -fsSL -o php.tar.xz.asc "$PHP_ASC_URL"; 		export GNUPGHOME="$(mktemp -d)"; 		for key in $GPG_KEYS; do 			gpg --batch --keyserver keyserver.ubuntu.com --recv-keys "$key"; 		done; 		gpg --batch --verify php.tar.xz.asc php.tar.xz; 		gpgconf --kill all; 		rm -rf "$GNUPGHOME"; 	fi; 		apt-mark auto '.*' > /dev/null; 	apt-mark manual $savedAptMark > /dev/null; 	apt-get purge -y --auto-remove -o APT::AutoRemove::RecommendsImportant=false
# Wed, 12 Apr 2023 19:55:07 GMT
COPY file:ce57c04b70896f77cc11eb2766417d8a1240fcffe5bba92179ec78c458844110 in /usr/local/bin/ 
# Wed, 12 Apr 2023 20:08:50 GMT
RUN set -eux; 		savedAptMark="$(apt-mark showmanual)"; 	apt-get update; 	apt-get install -y --no-install-recommends 		apache2-dev 		libargon2-dev 		libcurl4-openssl-dev 		libonig-dev 		libreadline-dev 		libsodium-dev 		libsqlite3-dev 		libssl-dev 		libxml2-dev 		zlib1g-dev 	; 		export 		CFLAGS="$PHP_CFLAGS" 		CPPFLAGS="$PHP_CPPFLAGS" 		LDFLAGS="$PHP_LDFLAGS" 	; 	docker-php-source extract; 	cd /usr/src/php; 	gnuArch="$(dpkg-architecture --query DEB_BUILD_GNU_TYPE)"; 	debMultiarch="$(dpkg-architecture --query DEB_BUILD_MULTIARCH)"; 	if [ ! -d /usr/include/curl ]; then 		ln -sT "/usr/include/$debMultiarch/curl" /usr/local/include/curl; 	fi; 	./configure 		--build="$gnuArch" 		--with-config-file-path="$PHP_INI_DIR" 		--with-config-file-scan-dir="$PHP_INI_DIR/conf.d" 				--enable-option-checking=fatal 				--with-mhash 				--with-pic 				--enable-ftp 		--enable-mbstring 		--enable-mysqlnd 		--with-password-argon2 		--with-sodium=shared 		--with-pdo-sqlite=/usr 		--with-sqlite3=/usr 				--with-curl 		--with-iconv 		--with-openssl 		--with-readline 		--with-zlib 				--disable-phpdbg 				--with-pear 				$(test "$gnuArch" = 's390x-linux-gnu' && echo '--without-pcre-jit') 		--with-libdir="lib/$debMultiarch" 				--disable-cgi 				--with-apxs2 	; 	make -j "$(nproc)"; 	find -type f -name '*.a' -delete; 	make install; 	find 		/usr/local 		-type f 		-perm '/0111' 		-exec sh -euxc ' 			strip --strip-all "$@" || : 		' -- '{}' + 	; 	make clean; 		cp -v php.ini-* "$PHP_INI_DIR/"; 		cd /; 	docker-php-source delete; 		apt-mark auto '.*' > /dev/null; 	[ -z "$savedAptMark" ] || apt-mark manual $savedAptMark; 	find /usr/local -type f -executable -exec ldd '{}' ';' 		| awk '/=>/ { print $(NF-1) }' 		| sort -u 		| xargs -r dpkg-query --search 		| cut -d: -f1 		| sort -u 		| xargs -r apt-mark manual 	; 	apt-get purge -y --auto-remove -o APT::AutoRemove::RecommendsImportant=false; 	rm -rf /var/lib/apt/lists/*; 		pecl update-channels; 	rm -rf /tmp/pear ~/.pearrc; 		php --version
# Wed, 12 Apr 2023 20:08:54 GMT
COPY multi:e11221d43af7136e4dbad5a74e659bcfa753214a9e615c3daf357f1633d9d3d1 in /usr/local/bin/ 
# Wed, 12 Apr 2023 20:09:00 GMT
RUN docker-php-ext-enable sodium
# Wed, 12 Apr 2023 20:09:04 GMT
ENTRYPOINT ["docker-php-entrypoint"]
# Wed, 12 Apr 2023 20:09:08 GMT
STOPSIGNAL SIGWINCH
# Wed, 12 Apr 2023 20:09:11 GMT
COPY file:e3123fcb6566efa979f945bfac1c94c854a559d7b82723e42118882a8ac4de66 in /usr/local/bin/ 
# Wed, 12 Apr 2023 20:09:14 GMT
WORKDIR /var/www/html
# Wed, 12 Apr 2023 20:09:18 GMT
EXPOSE 80
# Wed, 12 Apr 2023 20:09:22 GMT
CMD ["apache2-foreground"]
```
...
-	Layers:
	...
	-	`sha256:5cb687000899ee36170e2630950ff55713686987cdc618d935e1ca22fe2611a9`  
		...
		Size: 492.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:2ac0578428c35842ce55d1916936a9be945433b62bae871c4bbd3022d5487779`  
		Last Modified: Wed, 12 Apr 2023 22:37:55 GMT  
		Size: 11.9 MB (11901050 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:ece53c446ddbc7a72d6f9c89236af015d7ec8ce928cfb9b945cd09cccb8b4d90`  
		Last Modified: Wed, 12 Apr 2023 22:37:50 GMT  
		Size: 492.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:a35be99f572781e3a3126b9b4586e26fa0c7d1f4d23e8425022eed4d9a3ce35e`  
		Last Modified: Wed, 12 Apr 2023 22:37:57 GMT  
		Size: 10.2 MB (10213809 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:d7831f6d3881469807f94bb01eee07fb496acc7bb4fefef7f7694e1d0e2505df`  
		Last Modified: Wed, 12 Apr 2023 22:37:50 GMT  
		Size: 2.5 KB (2459 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:11d3206a10a23296f951f46d44a28bd762aaa09cfb960ecc6a5a361a0ff7ec4f`  
		Last Modified: Wed, 12 Apr 2023 22:37:50 GMT  
		Size: 247.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:bd03a0e2130e9bcc02f5f0883a62d5a9cea5755160faccbeb679f1da09f59d59`  
		Last Modified: Wed, 12 Apr 2023 22:37:50 GMT  
		Size: 896.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
