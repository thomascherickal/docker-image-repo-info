...
```console
$ docker pull neo4j@sha256:eadf7392b83ced87b50079d3006836b25a30d461d0fb3436d1e9b2b04ea6422d
```
...
```console
$ docker pull neo4j@sha256:6c7f4a908c8d999c291dd9c9757b695e76a7924652f603d45f83a6b77523b33b
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **367.5 MB (367529179 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:40b929e051a1b9bd1af1b02ebbbfe99c811004b879dcbeffaee05f28fc67f7f9`
...
-	Layers:
	...
	-	`sha256:e589a40108e12ad6a72d77e7543ab526530c495e6a55c520bee099db3e94ba32`  
		...
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:7cc3af52882ee21a30335c5514a42062d153cc66172a310809fdbf9060cbe1cc`  
		Last Modified: Mon, 25 Apr 2022 20:38:48 GMT  
		Size: 3.9 KB (3866 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:b34c441bba103bbab9422e814250ee9e6b5d8c7b3fa1f9f7bfc9fd08d575cef7`  
		Last Modified: Mon, 25 Apr 2022 20:38:48 GMT  
		Size: 605.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:2086c008d70393402ddce5855ff528472d6e5ea25847b4de7c76f59f2c1f2d1d`  
		Last Modified: Mon, 25 Apr 2022 20:38:55 GMT  
		Size: 130.3 MB (130317847 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:27241cc4aba181cb06ee21ba4c74f5f6981dc7409f1cec1946ec9e7207bd60fa`  
		Last Modified: Mon, 25 Apr 2022 20:38:49 GMT  
...
```console
$ docker pull neo4j@sha256:8ef58616d2579f34248cc41c3b5b46fb3ee9a0617267954d1622eedc8e2c9a09
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **363.5 MB (363504951 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c3fd54c20642ef30b501732082c3299798a1190347e1451a624d2fd4f42db6cf`
-	Entrypoint: `["\/sbin\/tini","-g","--","\/docker-entrypoint.sh"]`
-	Default Command: `["neo4j"]`
...
ENV NEO4J_SHA256=f0d36427fe4f646e5fca456fe9c584f0b970373574de4d57c9de9ff47df0a0e1 NEO4J_TARBALL=neo4j-community-4.4.0-unix.tar.gz NEO4J_EDITION=community NEO4J_HOME=/var/lib/neo4j
# Mon, 25 Apr 2022 22:10:33 GMT
ARG NEO4J_URI=https://dist.neo4j.org/neo4j-community-4.4.0-unix.tar.gz
# Mon, 25 Apr 2022 22:10:34 GMT
# ARGS: NEO4J_URI=https://dist.neo4j.org/neo4j-community-4.4.0-unix.tar.gz
RUN addgroup --gid 7474 --system neo4j && adduser --uid 7474 --system --no-create-home --home "${NEO4J_HOME}" --ingroup neo4j neo4j
# Mon, 25 Apr 2022 22:10:36 GMT
COPY multi:220c2663e29181f5cd4d1cc8ff4ea55bca5bd66e60ecec0890d11b2c206bdb54 in /tmp/ 
# Mon, 25 Apr 2022 22:10:50 GMT
# ARGS: NEO4J_URI=https://dist.neo4j.org/neo4j-community-4.4.0-unix.tar.gz
RUN apt update     && apt install -y curl wget gosu jq     && curl --fail --silent --show-error --location --remote-name ${NEO4J_URI}     && echo "${NEO4J_SHA256}  ${NEO4J_TARBALL}" | sha256sum -c --strict --quiet     && tar --extract --file ${NEO4J_TARBALL} --directory /var/lib     && mv /var/lib/neo4j-* "${NEO4J_HOME}"     && rm ${NEO4J_TARBALL}     && mv "${NEO4J_HOME}"/data /data     && mv "${NEO4J_HOME}"/logs /logs     && chown -R neo4j:neo4j /data     && chmod -R 777 /data     && chown -R neo4j:neo4j /logs     && chmod -R 777 /logs     && chown -R neo4j:neo4j "${NEO4J_HOME}"     && chmod -R 777 "${NEO4J_HOME}"     && ln -s /data "${NEO4J_HOME}"/data     && ln -s /logs "${NEO4J_HOME}"/logs     && mv /tmp/neo4jlabs-plugins.json /neo4jlabs-plugins.json     && set -eux; 	dpkgArch="$(dpkg --print-architecture)"; 	case "${dpkgArch##*-}" in 		'amd64')             tinisha="12d20136605531b09a2c2dac02ccee85e1b874eb322ef6baf7561cd93f93c855";             tiniurl="https://github.com/krallin/tini/releases/download/v0.18.0/tini"; 			;; 		'arm64')             tinisha="7c5463f55393985ee22357d976758aaaecd08defb3c5294d353732018169b019";             tiniurl="https://github.com/krallin/tini/releases/download/v0.18.0/tini-arm64"; 			;; 		*) echo >&2 "Neo4j does not currently have a docker image for architecture $dpkgArch"; exit 1 ;; 	esac     && curl -L --fail --silent --show-error ${tiniurl} > /sbin/tini     && echo "${tinisha}  /sbin/tini" | sha256sum -c --strict --quiet     && chmod +x /sbin/tini     && apt-get -y purge --auto-remove curl     && rm -rf /tmp/*     && rm -rf /var/lib/apt/lists/*
# Mon, 25 Apr 2022 22:10:50 GMT
ENV PATH=/var/lib/neo4j/bin:/usr/local/openjdk-11/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin
# Mon, 25 Apr 2022 22:10:51 GMT
WORKDIR /var/lib/neo4j
# Mon, 25 Apr 2022 22:10:52 GMT
VOLUME [/data /logs]
# Mon, 25 Apr 2022 22:10:54 GMT
COPY file:ab7ae7f90c4a2b48d55ce46bb4b2dcdc336a04742bc26d5311b62b6d1e2ce171 in /docker-entrypoint.sh 
# Mon, 25 Apr 2022 22:10:54 GMT
EXPOSE 7473 7474 7687
# Mon, 25 Apr 2022 22:10:55 GMT
ENTRYPOINT ["/sbin/tini" "-g" "--" "/docker-entrypoint.sh"]
# Mon, 25 Apr 2022 22:10:56 GMT
CMD ["neo4j"]
```
...
-	Layers:
	...
	-	`sha256:1e97b79cb6d1da2fc8e69b7cebb15a5eedc16448594417b3d3ba12fda8dec8c6`  
		...
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:1bc12e522b084dd2de5e047cc0fe4610c830fc8b58c869a44cab2a68a7166815`  
		Last Modified: Mon, 25 Apr 2022 22:17:19 GMT  
		Size: 3.8 KB (3844 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:b4f0b5ce1e05af467a815c7b3c5b88de69593c0af5e4c7ed2bc36374eaa48de8`  
		Last Modified: Mon, 25 Apr 2022 22:17:19 GMT  
		Size: 605.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:c92cf9d3e55bbf5ff1769891805edc77eda89b33d7c95bbcf1e70d02186057fd`  
		Last Modified: Mon, 25 Apr 2022 22:17:27 GMT  
		Size: 130.0 MB (130013718 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:f051728bfc7b8087e4d5da6f729a49d254866fe778d4d1267e47b40e488c84b0`  
		Last Modified: Mon, 25 Apr 2022 22:17:19 GMT  
