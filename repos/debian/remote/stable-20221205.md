## `debian:stable-20221205`

```console
$ docker pull debian@sha256:880aa5f5ab441ee739268e9553ee01e151ccdc52aa1cd545303cfd3d436c74db
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:stable-20221205` - linux; amd64

```console
$ docker pull debian@sha256:1f51b4ada92150468a245a7aca50710bff8b07b774e164d9136a8e00cc74a57a
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.0 MB (55041313 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a31188bc82b0db2e38b95282238d04e81de1f7c200edd71de4785c028e1d5e83`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 06 Dec 2022 01:22:13 GMT
ADD file:34ebed4832a7752758a57e6d077a2eedb22731d166f8b77e0b9f0339512d0705 in / 
# Tue, 06 Dec 2022 01:22:14 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:19a59d88084261156beed7ac55ecd91558f40d76259b7e7cd7173d88e6d3ab72`  
		Last Modified: Tue, 06 Dec 2022 01:27:09 GMT  
		Size: 55.0 MB (55041313 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20221205` - linux; arm variant v5

```console
$ docker pull debian@sha256:4d2019424caac012ac93addd1cabd5186c566b6b725b4a4a4e199462c4c92e35
```

-	Docker Version: 20.10.17
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.5 MB (52544808 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:53aebaf632e9122aae6274020914ab503eeed6b15986bcdf43225b5340d2e884`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 06 Dec 2022 01:50:05 GMT
ADD file:59fb9d758fc3ce22925cad8c3e6656e994e9fae9a37f70145cdc0219b1fbd2a8 in / 
# Tue, 06 Dec 2022 01:50:06 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:cc47e2cad097381f4c9183b162a2981f5606b7863cbf8a89175d2520e9bff8af`  
		Last Modified: Tue, 06 Dec 2022 01:55:36 GMT  
		Size: 52.5 MB (52544808 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20221205` - linux; arm variant v7

```console
$ docker pull debian@sha256:70a648cd42c459cfe290c2297746fe5b8f6aaea43ba3d7bcb62b4a65f8793528
```

-	Docker Version: 20.10.17
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.2 MB (50207056 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fe380e7f3114c18bf5dd1cb6bb48fb433918b9f22077fd6fd201f9deec51bea3`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 06 Dec 2022 01:00:44 GMT
ADD file:34f9baa882dfd11b8d0ee9cc3d1c369289f5d47eb1f07aaf8d8b18eeea6477be in / 
# Tue, 06 Dec 2022 01:00:45 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5d1b0930d4cad36cd9f85cc4f500e9c114edc3ba6d1080757eaba32a04c42d53`  
		Last Modified: Tue, 06 Dec 2022 01:08:32 GMT  
		Size: 50.2 MB (50207056 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20221205` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:9d90fba0cd4928ae7ab842c41b6cecd60305207a37e78c922fa7365082c5019b
```

-	Docker Version: 20.10.17
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.7 MB (53699150 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f633f9d7169dc440c57ae6dbe3f3ba0e96fd7697026b18b032a5280ae5a1e7e3`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 06 Dec 2022 01:41:13 GMT
ADD file:758ca4d53492adae96ec2aaa1ea3c794f43083eca557a2e01e8b81d915c02ceb in / 
# Tue, 06 Dec 2022 01:41:13 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e5c57f815a1ca5f6cd30e9cb0cc0d6870e7cc286ebb9a31541929bdf11ec4516`  
		Last Modified: Tue, 06 Dec 2022 01:45:48 GMT  
		Size: 53.7 MB (53699150 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20221205` - linux; 386

```console
$ docker pull debian@sha256:250560fe914d3669f16d7295a17539ff4c1c2e2b2bf3c9da0429419f4fc99f5a
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.0 MB (56022384 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:250eba3d72d7c12f308de96ba01e0cda3897ea487e4a40244bb74ef870ef47a8`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 06 Dec 2022 01:41:26 GMT
ADD file:a443dfaae8d0f9292fe3697cb02f3ceb3749f79f691672825ec5f3efd7fdd674 in / 
# Tue, 06 Dec 2022 01:41:27 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:bd17635423722978b706617da5ee23821e711a5c6e21efa6abc76d52fc979193`  
		Last Modified: Tue, 06 Dec 2022 01:48:04 GMT  
		Size: 56.0 MB (56022384 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20221205` - linux; mips64le

```console
$ docker pull debian@sha256:ea14f92ade538f94ac0f4c03e4dc6ddf84b24b12e498179f984f5bb0c38304b5
```

-	Docker Version: 20.10.17
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53260610 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:eab7582473001ba47b29b20966450d6eaa2296bc627095254618a3fef783f1be`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 06 Dec 2022 01:57:15 GMT
ADD file:879ab6f79e2fb8d3fd17adfe0473c543a636d57831277f7d7f0626ea24e560a2 in / 
# Tue, 06 Dec 2022 01:57:20 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6634ea6ae10263eeb0de0e54f17261eb4245a26bfcb5e1a46bcb04ed57e6fc6c`  
		Last Modified: Tue, 06 Dec 2022 02:05:32 GMT  
		Size: 53.3 MB (53260610 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20221205` - linux; ppc64le

```console
$ docker pull debian@sha256:d118174180a33952252a423a955f3f3718735517e5e0dbd04cb8fc764d23e657
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **58.9 MB (58913126 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3bd17284165a5f0fceb98a489c5171565ef1daa8ee35dab11a69ca2bce145679`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 06 Dec 2022 01:19:04 GMT
ADD file:52359c01b57648275662cd3780e749e801fc2fb35889ba101416aa560bd9dc78 in / 
# Tue, 06 Dec 2022 01:19:07 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:9e0e7d92200dbf05b61c73eedc3538266a8a5696ba36cd3dde0f961483c6415d`  
		Last Modified: Tue, 06 Dec 2022 01:25:31 GMT  
		Size: 58.9 MB (58913126 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20221205` - linux; s390x

```console
$ docker pull debian@sha256:d36156fee9aeead88ae4f129a8a76f56bf5459197bba2334f30005c22bee984b
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53272907 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:dfe1cd90544b4b6a7e8db2f8d13efbb327289bce07e16991a3011a97a8c91e9c`
-	Default Command: `["bash"]`

```dockerfile
# Tue, 06 Dec 2022 01:44:04 GMT
ADD file:8c4ea37efa7dd75ec75b961a8b78219f331b5e48424492afd8d42558b16f9dd0 in / 
# Tue, 06 Dec 2022 01:44:10 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:9fe7412e88822f809fce61091bc947285ba4905570a884732c61f03a6ed3354c`  
		Last Modified: Tue, 06 Dec 2022 01:49:25 GMT  
		Size: 53.3 MB (53272907 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
