## `debian:stable-20230320`

```console
$ docker pull debian@sha256:d4bbca2152d6376b59f3dd85a73e6fa041693f8a5f2919c47d4f718fc1a2878f
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:stable-20230320` - linux; amd64

```console
$ docker pull debian@sha256:729c2433e196207749a86f1d86e0106822041bb280b4200cf7a4db97608f6d3a
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.0 MB (55045577 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3228674897d15947710ca1d2898b4a9809b026a1098a3e8b3f604c461172466c`
-	Default Command: `["bash"]`

```dockerfile
# Thu, 23 Mar 2023 01:31:47 GMT
ADD file:bdf2c9eedba0acd35488f592044047931dc5a13a4f0a6923783cbc1ed9657699 in / 
# Thu, 23 Mar 2023 01:31:47 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:4ed5f2fb37e281a299382e8fab2bbc22af2994dd30464f32e0f21b5200806a5e`  
		Last Modified: Thu, 23 Mar 2023 01:36:20 GMT  
		Size: 55.0 MB (55045577 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20230320` - linux; arm variant v5

```console
$ docker pull debian@sha256:30abf209008d7fadf1d28e005f2bdb1cf8df662845bf3c23bdd5759825b71d92
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.5 MB (52549504 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:81f0aaa9bf557b5fa85f860f18c53567bccbd7592ff52e8da1fa9fb2ca161472`
-	Default Command: `["bash"]`

```dockerfile
# Thu, 23 Mar 2023 00:49:05 GMT
ADD file:30388438fd6b8a68ceffc950d98fc11963f9f317825ab87d786d042e8cd97f7f in / 
# Thu, 23 Mar 2023 00:49:06 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a5af868c4b3a926efdb22a9a3b56d17995390b940f566eeaa9b8781cca2f96dc`  
		Last Modified: Fri, 24 Mar 2023 23:22:39 GMT  
		Size: 52.5 MB (52549504 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20230320` - linux; arm variant v7

```console
$ docker pull debian@sha256:2bc8a2d2eb68110c56034957f360d60fdd08323c17c4ad1ab8a14f1c82ebf5e3
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.2 MB (50211805 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e9215ea5449ebeb6cd872b65f962e44b2264e6bde21cf235aaba1d291738106c`
-	Default Command: `["bash"]`

```dockerfile
# Thu, 23 Mar 2023 01:30:31 GMT
ADD file:75c6ccb0c507d5995bf9631eeb29804be7fd8ed92aeb067d47ab99157f95fae7 in / 
# Thu, 23 Mar 2023 01:30:32 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:fecb0fab7764093a84528aee6bbeedad85dc45ae0dcdd714ddd83c34318c5c76`  
		Last Modified: Thu, 23 Mar 2023 01:34:36 GMT  
		Size: 50.2 MB (50211805 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20230320` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:8b7336b66767dee228583e6740a1a75f9cc8d42c7d5b4e72267164ecf7625b53
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.7 MB (53703140 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6c76aad82e8bc9f188baaf07d5b81ffe1213e2315aca94ff4553cd73ef31c37f`
-	Default Command: `["bash"]`

```dockerfile
# Thu, 23 Mar 2023 00:45:55 GMT
ADD file:3b227c85bf0970aced33c4da102d7d282e814bf53a84d6a6ef7f581bbc76a7bf in / 
# Thu, 23 Mar 2023 00:45:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2312bb915b25efe9ce812b12ae65ea209b43c9ef384f3295eb1c663aa11a97b7`  
		Last Modified: Thu, 23 Mar 2023 00:50:01 GMT  
		Size: 53.7 MB (53703140 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20230320` - linux; 386

```console
$ docker pull debian@sha256:db4bfadd26a27ad51e95eae043ff4fad570ffb2461e43b4817a147400f2f8f4d
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.0 MB (56027847 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a46eaf4bff16c2bc7fec168edf80afc01600bd8333bd94d818917a0077cc6c48`
-	Default Command: `["bash"]`

```dockerfile
# Thu, 23 Mar 2023 00:40:31 GMT
ADD file:7e6282db7e6f5f89339f56dff6a7ce78d25479aad91e687d59b76948cd43af7c in / 
# Thu, 23 Mar 2023 00:40:31 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:255456fff76dbf9560f73643299d3b99f21e9ec4f1129ea6904f33328be0add3`  
		Last Modified: Thu, 23 Mar 2023 00:45:26 GMT  
		Size: 56.0 MB (56027847 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20230320` - linux; mips64le

```console
$ docker pull debian@sha256:8cf9d9df6c5fe9dee16f9beb6ed8621909ad30bf227956d92830f724227b70ed
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53264695 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4df86c9c08f197b4ed845d23bdd6573c857e187187073b0909cd965bd22e960b`
-	Default Command: `["bash"]`

```dockerfile
# Thu, 23 Mar 2023 05:19:35 GMT
ADD file:ad71d18fed9a54f08962ed260895e248d5632b0529e80c990e43ecf453414e3f in / 
# Thu, 23 Mar 2023 05:19:41 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1b456ecc6382bcff949011c0e1791c82dd243ee3837a15605c016951144bee49`  
		Last Modified: Thu, 23 Mar 2023 05:27:42 GMT  
		Size: 53.3 MB (53264695 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20230320` - linux; ppc64le

```console
$ docker pull debian@sha256:f8de1ff2d027131ee773f52d1de04818c588ecb0e6bb7b80b4fa201545981f7e
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **58.9 MB (58921027 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cf337cafef50281e9212899500a58090eaaf1c52b99d8d77ded63b55f124af3d`
-	Default Command: `["bash"]`

```dockerfile
# Thu, 23 Mar 2023 01:20:43 GMT
ADD file:d51838f4a37d18901518b506ce14df54086071003c6efa1c9aeae7efdea2d348 in / 
# Thu, 23 Mar 2023 01:20:46 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:beddbecfd1432bd897bb0f2b9cba7063e832605b0e0fbe4bcc4b7d89bd936994`  
		Last Modified: Thu, 23 Mar 2023 01:25:28 GMT  
		Size: 58.9 MB (58921027 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20230320` - linux; s390x

```console
$ docker pull debian@sha256:0e4790c2dea183778b796be1af67cd44217336f09f456b771f7910909c2b52ff
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.3 MB (53277458 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4ec73c7e9f95d35b25b49003ded10093c8f5140706f343523a7104930c9ca5fd`
-	Default Command: `["bash"]`

```dockerfile
# Thu, 23 Mar 2023 00:44:41 GMT
ADD file:0a147643a8506b1b4bccf8b33cb7a3868d9279381b0cd1da59e01d3eb3e3490c in / 
# Thu, 23 Mar 2023 00:44:46 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f5e53e6370cdd67572dcf9ec5a8f5998c3025aff2de4beadf65f001656040863`  
		Last Modified: Thu, 23 Mar 2023 00:47:52 GMT  
		Size: 53.3 MB (53277458 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
